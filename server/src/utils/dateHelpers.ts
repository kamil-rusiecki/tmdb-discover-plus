...
export function resolveDynamicDatePreset(
  ...
  switch (filters.datePreset) {
    ...
    default: {
      const eraMatch = String(filters.datePreset).match(/^era_(\d{4})s$/);
      if (eraMatch) {
        const decade = parseInt(eraMatch[1], 10);
        resolved[fromField] = `${decade}-01-01`;
        resolved[toField] = `${decade + 9}-12-31`;
      }
      break;
    }
  }
